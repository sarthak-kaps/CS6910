...
class RMSprop(Optimizer):
    ...
    def apply_gradients(self, weight_gradient, bias_gradient, weight, bias, epoch):
        ...
        self.weight_velocity = self.beta * self.weight_velocity + \
            (1 - self.beta) * np.square(weight_gradient)
        self.bias_velocity = self.beta * self.bias_velocity + \
            (1 - self.beta) * np.square(bias_gradient)

        new_weight = weight - self.learning_rate * \
            np.reciprocal(np.sqrt(self.weight_velocity +
                                  self.eps)) * weight_gradient
        new_bias = bias - self.learning_rate * \
            np.reciprocal(np.sqrt(self.bias_velocity + self.eps)
                          ) * bias_gradient
...
class Adam(Optimizer):
    ...
    def apply_gradients(self, weight_gradient, bias_gradient, weight, bias, epoch):
        ...
        self.weight_momentum = beta_1 * self.weight_momentum + \
            (1 - beta_1) * weight_gradient
        self.bias_momentum = beta_1 * self.bias_momentum + \
            (1 - beta_1) * bias_gradient

        self.weight_velocity = beta_2 * self.weight_velocity + \
            (1 - beta_2) * np.square(weight_gradient)
        self.bias_velocity = beta_2 * self.bias_velocity + \
            (1 - beta_2) * np.square(bias_gradient)

        weight_momentum_hat = self.weight_momentum * \
            (1 / (1 - np.power(beta_1, epoch)))
        bias_momentum_hat = self.bias_momentum * \
            (1 / (1 - np.power(beta_1, epoch)))

        weight_velocity_hat = self.weight_velocity * \
            (1 / (1 - np.power(beta_2, epoch)))
        bias_velocity_hat = self.bias_velocity * \
            (1 / (1 - np.power(beta_2, epoch)))

        new_weight = weight - self.learning_rate * \
            np.reciprocal(np.sqrt(weight_velocity_hat + self.eps)
                          ) * weight_momentum_hat
        new_bias = bias - self.learning_rate * \
            np.reciprocal(np.sqrt(bias_velocity_hat + self.eps)
                          ) * bias_momentum_hat
...
class Nadam(Optimizer):
    ...
    def apply_gradients(self, weight_gradient, bias_gradient, weight, bias, epoch):
        ...
        weight_gradient_nu = (
            (1 - beta_1) / (1 - np.power(beta_1, epoch))) * weight_gradient
        bias_gradient_nu = (
            (1 - beta_1) / (1 - np.power(beta_1, epoch))) * bias_gradient

        new_weight = weight - self.learning_rate * np.reciprocal(np.sqrt(
            weight_velocity_hat + self.eps)) * (weight_momentum_hat_nu + weight_gradient_nu)
        new_bias = bias - self.learning_rate * \
            np.reciprocal(np.sqrt(bias_velocity_hat + self.eps)) * \
