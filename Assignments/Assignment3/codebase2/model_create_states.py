...
def stack_ragged(tensors):
    ...
    lens = tf.stack([tf.shape(t, out_type=tf.float64)[0] for t in tensors])
...
class Encoder(tf.keras.Model):
    ...
    def call(self, x):
        ...
        for i in range(0, len(self.model)):
            ...
            if type(layer_out) != list :
                states.extend([layer_out[1:]])
            else :
                states.extend(layer_out[1:])

        output = x
        #states = stack_ragged(states)
...
class Decoder(tf.keras.Model):
    ...
    def call(self, x, hidden, enc_output, states):
        ...
        factor = 2 ** (self.config.cell_type == "LSTM")
        ...
        if self.config.attention:
            context_vector, attention_weights = self.attention(
                hidden, enc_output)
            # x shape after concatenation == (batch_size, 1, embedding_im + hidden_size)
        ...
        for i in range(0, len(self.model)):
            self.model[i].initial_state = states[factor * i : factor * (i + 1)]
